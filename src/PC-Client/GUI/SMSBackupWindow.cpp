...
void SMSBackupWindow::on_button_backup_sms_click() {
    ...
    QObject::connect(thread, SIGNAL(backup_sms_complete(bool, QString)), this,
                     SLOT(on_sms_backup_complete(bool, QString)));
    thread->start();
    scan_progress_dialog->exec();
}
...
void SMSBackupWindow::set_tree_model() {
    ...
    QObject::connect(model, SIGNAL(itemChanged(QStandardItem * )), this,
                     SLOT(on_tree_view_item_check_change(QStandardItem * )));
    sms_group_list = sms_list_sort();
    for (int i = 0; i < sms_group_list.size(); i++) {
        sprintf(temp, "%s(%d条)", sms_group_list[i].phone_number.toStdString().c_str(),
                (int) sms_group_list[i].sms_index.size());
        ...
        for (int j = 0; j < sms_group_list[i].sms_index.size(); j++) {
            QStandardItem *sms_date_item = new QStandardItem(QIcon(":/icon/clock.svg"), QString::fromStdString(
                    sms_list[sms_group_list[i].sms_index[j]].get_date_str()));
            sms_date_item->setEditable(false);
            sms_date_item->setCheckable(true);
            QStandardItem *sms_body_item = new QStandardItem(
                    QString::fromStdString(sms_list[sms_group_list[i].sms_index[j]].get_sms_body()));
...
void SMSBackupThread::run() {
    emit backup_sms_complete(SMSAndContactBackup::export_sms(backup_path.c_str(), *sms_list),
