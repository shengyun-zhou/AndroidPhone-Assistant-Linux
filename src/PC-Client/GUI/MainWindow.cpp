...
MainWindow::MainWindow(ADBTools *tools, QWidget *parent) : QMainWindow(parent), ui(new Ui::MainWindow) {
    ...
    QObject::connect(ui->button_connect_to_phone, SIGNAL(clicked(bool)), this,
                     SLOT(on_button_connect_to_phone_click()));
...
void MainWindow::on_connect_complete(bool connect_result) {
    ...
    if (!connect_result)
        ...
        char temp[1024];
        sprintf(temp, "<h1>%s</h1><p><span style=\"font-size:15pt;\">%s（API%d）</span></p>", new_phone_info.c_str(),
                android_version.c_str(), info.get_sdk_version());
